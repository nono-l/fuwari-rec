...
/**
 * Fetch the deployer-injected canonical origin (`BETTER_AUTH_URL`). Used to
 ...
 *   If the page is on a custom domain different from BETTER_AUTH_URL, we first
 *   bounce to the canonical host so OAuth cookies + redirect_uri stay aligned,
 *   then bridge the session back with a one-time token.
 *
...
/** Sign out (cookie path + any stored preview bearer). */
