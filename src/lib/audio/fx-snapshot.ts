...
function insertXml(f: ObsInsert) {
  ...
  const u = normalizeUpwardTune(f.upwardTune, f.amount);
...
function parseFilterEl(f: Element): SpectrumFilter | null {
  return normalizeFilter({
    ...
    formant: normalizeFormantTune({
      f1Hz: num(attr(f, "formantF1"), 700),
      f2Hz: num(attr(f, "formantF2"), 1200),
      f3Hz: num(attr(f, "formantF3"), 2500),
      q1: num(attr(f, "formantQ1"), 1.2),
      q2: num(attr(f, "formantQ2"), 1),
      q3: num(attr(f, "formantQ3"), 0.85),
      gender: num(attr(f, "formantGender"), 0.5),
      mix: num(attr(f, "formantMix"), 1),
    }),
    deess: normalizeDeessTune({
      thresholdDb: num(attr(f, "deessThresh"), -24),
  ...
  return normalizeObsInsert({
    ...
    upwardTune:
      attr(f, "upThresh") === ""
        ? undefined
        : {
            thresholdDb: num(attr(f, "upThresh"), -18),
            ratio: num(attr(f, "upRatio"), 2),
            attackMs: num(attr(f, "upAtk"), 8),
            releaseMs: num(attr(f, "upRel"), 100),
            mix: num(attr(f, "upMix"), 1),
          },
    expanderTune:
      attr(f, "expThresh") === ""
        ? undefined
        : {
            thresholdDb: num(attr(f, "expThresh"), -40),
            ratio: num(attr(f, "expRatio"), 2),
            attackMs: num(attr(f, "expAtk"), 5),
            releaseMs: num(attr(f, "expRel"), 80),
            mix: num(attr(f, "expMix"), 1),
          },
    denoiseTune:
      attr(f, "denoiseAtk") === ""
        ? undefined
        : {
            mix: num(attr(f, "denoiseMix"), 1),
            attack: num(attr(f, "denoiseAtk"), 0.4),
            gateLink: attr(f, "denoiseGate") === "true",
            thresholdDb: num(attr(f, "denoiseThresh"), -38),
          },
    howlTune:
      attr(f, "howlDepth") === ""
        ? undefined
