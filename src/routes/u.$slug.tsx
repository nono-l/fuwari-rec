...
export const Route = createFileRoute("/u/$slug")({
  ...
  head: ({ params }) => ({
    meta: [{ title: `${params.slug} — Fuwari REC` }],
  }),
});
...
function PublicProfilePage() {
  ...
  useEffect(() => {
    void getPublicProfile({ data: slug }).then(setProfile);
  }, [slug]);
    ...
    return (
      <main className="grid min-h-dvh place-items-center bg-background px-4 pt-[var(--grok-banner-h,0px)]">
        <div className="max-w-sm text-center">
          <p className="text-sm text-muted-foreground">この公開ページはありません</p>
